...
private:
  ...
  double square_step_ = 0.3; // or less for more accurate result (but not less 0.1)
  ...
  int calculate_progress() {
    if (task_type_ == "return_home") {
      ...
      if (square_state_ == SquareState::COMPLETE) return 100;
      
      double progress = 0.0;
      
      switch (square_state_) {
      case SquareState::MOVE_RIGHT:
        progress = (current_pose_.x - start_x_) / area_size_ * 25.0;
        break;
      case SquareState::MOVE_UP:
        progress = 25.0 + (current_pose_.y - start_y_) / area_size_ * 25.0;
        break;
      case SquareState::MOVE_LEFT:
        progress = 50.0 + (1.0 - (current_pose_.x - start_x_) / area_size_) * 25.0;
        break;
      case SquareState::MOVE_DOWN:
        progress = 75.0 + (1.0 - (current_pose_.y - start_y_) / area_size_) * 25.0;
        break;
      default:
        progress = 0.0;
      }
      
      if (total_square_passes_ > 0) {
        double passes_progress = (100.0 * square_passes_) / total_square_passes_;
        progress = std::min(passes_progress + progress * (1.0 / total_square_passes_), 100.0);
      }
      
      return std::min(100, std::max(0, static_cast<int>(progress)));
    }
    return 0;
  }
